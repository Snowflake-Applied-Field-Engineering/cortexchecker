"""
Cortex Unified Tool - Combined Role Checker & Agent Permission Generator
...
st.set_page_config(
    layout="wide", 
    page_title="Cortex Unified Tool",
    page_icon="🔐"
)
...
def generate_agent_permission_sql(agent_name, database, schema, tools, semantic_views_data, role_name=None):
    ...
    sql_lines.append(f"-- Generated by: Cortex Unified Tool")
    sql_lines.append("-- " + "="*80)
    sql_lines.append("")
                    ...
                    with st.expander(f"📋 Analysis: {role_name}", expanded=len(selected_roles) == 1):
                        ...
                        if not grants_df.empty:
                            ...
                            if progress_pct == 1.0:
                                ...
                                st.warning(f"**⚠️ MOSTLY READY** - Score: {readiness_score}/4")
                            else:
                ...
                with st.spinner("Analyzing..."):
                    ...
                    if not grants_df.empty and agent_info:
                        ...
                        if has_agent_access and has_cortex and has_warehouse:
                            ...
                            st.warning("⚠️ **Role needs additional permissions**")
